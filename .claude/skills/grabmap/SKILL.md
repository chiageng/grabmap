---
name: grabmap
description: Reference for building with GrabMaps — the `grab-maps` JS/TS library (MapLibre-based), HTTP gateway endpoints (places, POI search, nearby, reverse geocode, directions/ETA, coverage tiles, sessions), map style loading via `GET https://maps.grab.com/api/style.json` with Bearer auth, and MCP tools (`search_places`, `nearby_search`, `get_directions`). Use when writing code that calls `maps.grab.com`, imports `grab-maps`, uses `GrabMapsBuilder`/`GrabMapsLib`/`MapBuilder`, initializes MapLibre against Grab tiles, or works with API keys shaped like `bm_…`.
---
